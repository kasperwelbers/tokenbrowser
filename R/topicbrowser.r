function(){
  ...
  tokens = merge(tokens, word_assignments, by=c('doc_id','lemma'), all.x=T, sort = F)

  doc_topic = doc_topic_mat
...
match_word_assignments <- function(tokens, word_assignments, topic_docs, doc_col, feature_col) {
  ...
  tokens = tokens[tokens[[doc_col]] %in% doc_filter, c(doc_col, token_col, feature_col),]
...
choose_topic_docs <- function(doc_topic, docs_per_topic, doc_select) {
  ...
  best_doc_topic = split(setNames(best_doc_topic$score, nm = as.character(best_doc_topic$doc_id)),
                         best_doc_topic$topic)

  if (!is.na(docs_per_topic)) best_doc_topic = sapply(best_doc_topic, head, n = docs_per_topic, simplify = F)
  best_doc_topic
}
